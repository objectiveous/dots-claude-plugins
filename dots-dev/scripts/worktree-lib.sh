...
# =============================================================================
# Dots UI Session Registry (~/.dots/sessions.json)
# These functions register/unregister sessions for visibility in the Dots UI
# =============================================================================

# Get the Dots session registry file path
get_dots_registry_file() {
  echo "$HOME/.dots/sessions.json"
}

# Ensure Dots session registry exists
ensure_dots_registry() {
  local registry_file
  registry_file=$(get_dots_registry_file)
  mkdir -p "$(dirname "$registry_file")"
  [ ! -f "$registry_file" ] && echo '{"version": 1, "sessions": {}}' > "$registry_file"
}

# Register a session in ~/.dots/sessions.json for UI visibility
# Usage: register_dots_session "servus-bead-123" "bead-123" "/path/to/worktree" "feature/bead-123"
register_dots_session() {
...
# Unregister a session from ~/.dots/sessions.json
unregister_dots_session() {
  local session_name="$1"
  local registry_file
  ...
  if [ -f "$registry_file" ]; then
    jq --arg id "$session_name" 'del(.sessions[$id])' "$registry_file" > "$registry_file.tmp" \
      && mv "$registry_file.tmp" "$registry_file"
  fi
}
...
kill_tmux_session() {
  ...
  if [ -n "$session_name" ]; then
    tmux kill-session -t "$session_name" 2>/dev/null || {
      echo "Warning: Could not kill tmux session $session_name (may already be closed)"
    }
  fi
}
